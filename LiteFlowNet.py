...
def Backward(tensorInput, tensorFlow):
    ...
    tensorFlow = torch.cat([ tensorFlow[:, 0:1, :, :].clone() / ((tensorInput.size(3) - 1.0) / 2.0), tensorFlow[:, 1:2, :, :].clone() / ((tensorInput.size(2) - 1.0) / 2.0) ], 1)
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        class Regularization(torch.nn.Module):
            ...
            def forward(self, tensorFirst, tensorSecond, tensorFeaturesFirst, tensorFeaturesSecond, tensorFlow):
                ...
                tensorScaleX = self.moduleScaleX(tensorDist * torch.nn.functional.unfold(input=tensorFlow[:, 0:1, :, :].clone(), kernel_size=self.intUnfold, stride=1, padding=int((self.intUnfold - 1) / 2)).view_as(tensorDist)) * tensorDivisor
                tensorScaleY = self.moduleScaleY(tensorDist * torch.nn.functional.unfold(input=tensorFlow[:, 1:2, :, :].clone(), kernel_size=self.intUnfold, stride=1, padding=int((self.intUnfold - 1) / 2)).view_as(tensorDist)) * tensorDivisor
    ...
    def forward(self, tensorFirst, tensorSecond):
        tensorFirst[:, 0, :, :] = tensorFirst[:, 0, :, :].clone() - 0.411618
        tensorFirst[:, 1, :, :] = tensorFirst[:, 1, :, :].clone() - 0.434631
        tensorFirst[:, 2, :, :] = tensorFirst[:, 2, :, :].clone() - 0.454253

        tensorSecond[:, 0, :, :] = tensorSecond[:, 0, :, :].clone() - 0.410782
        tensorSecond[:, 1, :, :] = tensorSecond[:, 1, :, :].clone() - 0.433645
        tensorSecond[:, 2, :, :] = tensorSecond[:, 2, :, :].clone() - 0.452793
...
def estimate(tensorFirst, tensorSecond):
    ...
    tensorFlow[:, 0, :, :] = tensorFlow[:, 0, :, :].clone() * float(intWidth) / float(intPreprocessedWidth)
    tensorFlow[:, 1, :, :] = tensorFlow[:, 1, :, :].clone() * float(intHeight) / float(intPreprocessedHeight)

    return tensorFlow[0, :, :, :].clone().cpu()
# end
...
def batch_estimate(tensor_batch,moduleNetwork):
    ...
    tensorFirst=tensor_batch[:,:3,].clone()
    tensorSecond=tensor_batch[:,3:,].clone()
